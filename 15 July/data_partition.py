...
Original file is located at
    https://colab.research.google.com/drive/1OXWcSkh_vePS5r2Txc-QNxZmm9n8JasO
"""
...
def get_dir_label(data_dir):
    cate_list = os.listdir(os.path.join(data_dir, 'images'))
    if '.DS_Store' in cate_list:
        cate_list.remove('.DS_Store')
    class_id_dict = {}
    for i in range(len(cate_list)):
        current_class = cate_list[i]
        class_dir = os.path.join(data_dir, 'images', current_class)
...
train_sample, train_label, val_sample, val_label, test_sample, test_label=data_construction(data_dir='/content/drive/MyDrive/ImageBind finetune/ImageBind-LoRA/data')

num_category={}
for item in val_sample:
  category=item.split('/')[0]
  if category not in num_category:
    num_category[category] = 0
  num_category[category] += 1
